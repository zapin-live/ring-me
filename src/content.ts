...
class Beeper {
  ...
  private audioOscillator;

  constructor() {
    const audio = new (window.AudioContext || (window as any).webkitAudioContext)()
    this.audioOscillator = audio.createOscillator()
    this.audioOscillator.connect(audio.destination);
  }
  ...
  beep = async (frequency: number, duration: number) => {
    ...
    this.audioOscillator.frequency.value = frequency; // value in hertz
    this.audioOscillator.start();

    setTimeout(() => { this.audioOscillator.stop(); }, duration);
  }
