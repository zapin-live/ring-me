...
const main = async () => {
  ...
  receiveMessage("activation-toggled", async () => {
    const isActive = await db.get("isActive") && !await db.get("disabledUntil") && !beeper.isTemporarilyDisabled;
...
class Beeper {
  ...
  beep = async (frequency: number, duration: number) => {
    ...
    chrome.runtime.sendMessage({ type: 'playSound', volume: this.volume, frequency, duration });
  }
